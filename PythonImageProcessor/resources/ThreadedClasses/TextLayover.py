...
class TextLayoverThread(ThreadedClass):
    ...
    def __init__(self,
                 ...
                 start_of_race_timestamp: datetime = datetime.strptime('2020-07-11 12:41:00', '%Y-%m-%d %H:%M:%S'),
                 dock_leave_timestamp: datetime = datetime.strptime('2020-07-11 10:15:00', '%Y-%m-%d %H:%M:%S'),
                 end_of_race_timestamp: datetime = datetime.strptime('2020-07-14 10:00:00', '%Y-%m-%d %H:%M:%S')):
        super().__init__()
    ...
    def run(self):
        ...
        # opencv processing
        if self.track_data.timestamp.hour > 21 or self.track_data.timestamp.hour < 6:
            ...
            try:
                ...
                pass
                # self.logger.error(f'Error while modifying image: {e}')
        ...
        try:
            ...
            left_justification = image_x - (character_width * character_count)
            top_justification = image_y - (line_height * line_count)
            shape = [(left_justification, top_justification), (image_x, image_y)]
    ...
    @staticmethod
    def automatic_brightness_and_contrast(image: Image, clip_hist_percent: int = 10) -> 'auto_result, alpha, beta':
